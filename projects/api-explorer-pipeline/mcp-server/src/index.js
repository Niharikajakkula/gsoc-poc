...
class ApiExplorerClient {
  ...
  async searchApis(query) {
    try {
      ...
      const response = await fetch(`${this.baseUrl}/agent/tools/search`, {
        method: 'POST',
  ...
  async listApis() {
    try {
      ...
      const response = await fetch(`${this.baseUrl}/agent/tools/list`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({}),
      });
  ...
  async executeApi(method, path, api) {
    try {
      ...
      const response = await fetch(`${this.baseUrl}/agent/tools/execute`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
  ...
  async testConnection() {
    try {
      ...
      if (response.ok) {
        ...
        console.error(`❌ [MCP] Backend responded with status: ${response.status}`);
        return false;
      }
    } catch (error) {
    ...
    this.server.setRequestHandler(ListToolsRequestSchema, async () => {
      ...
      return {
        tools: [
          {
            name: 'search_apis',
            description: 'Search for APIs using natural language queries. Returns the best matching API endpoint with templates and alternatives.',
            inputSchema: {
              type: 'object',
              properties: {
                query: {
                  type: 'string',
                  description: 'Natural language query to search for APIs (e.g., "get users", "create pet", "weather forecast")',
                },
              },
              required: ['query'],
            },
          ...
          {
            name: 'list_apis',
            description: 'List all available APIs in the registry with their metadata, categories, and endpoint counts.',
            inputSchema: {
              type: 'object',
              properties: {},
            },
          ...
          {
            name: 'execute_api',
            description: 'Execute an API endpoint with mock responses for testing and demonstration purposes.',
            inputSchema: {
              type: 'object',
              properties: {
                method: {
                ...
                path: {
                  type: 'string',
                  description: 'API endpoint path (e.g., "/users", "/pets/{id}")',
                },
                api: {
                  type: 'string',
      ...
      try {
        ...
        switch (name) {
          ...
          case 'execute_api': {
            ...
            if (!method || !path || !api) {
              throw new Error('Method, path, and api parameters are all required');
            }
