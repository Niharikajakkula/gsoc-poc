...
class ApiExplorerClient {
  ...
  constructor(baseUrl: string = API_EXPLORER_BASE_URL) {
    this.baseUrl = baseUrl;
  }

  async searchApis(query: string): Promise<any> {
    try {
  ...
  async executeApi(method: string, path: string, api: string): Promise<any> {
    try {
...
class ApiExplorerMcpServer {
  ...
  private client: ApiExplorerClient;
    ...
    this.server.setRequestHandler(CallToolRequestSchema, async (request) => {
      ...
      try {
        ...
        switch (name) {
          ...
          case 'execute_api': {
            const { method, path, api } = args as { method: string; path: string; api: string };
            if (!method || !path || !api) {
